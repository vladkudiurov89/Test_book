...
plt.plot(diabetes_X_test, diabetes_Y_predict, color="red")
...
reg_2.fit([[0, 0], [0, 0], [1, 1]], [0, .1, 1])
# print(reg_2)
# print("coefficient: ", reg_2.coef_)
# print("intercept: ", reg_2.intercept_)

# Example
# Plot Ridge coefficients as a function of the regularization
# x is the Hilbert matrix 10*10
...
# Функция np.logspace() возвращает одномерный массив из указанного количества элементов,
# значения которых равномерно распределенны по логарифмической шкале внутри заданного интервала.
...
op.add_option("--chi2_select", action="store", type="int", dest="select_chi2", help="Select some number of features "
                                                                                    "using a chi-squared test")
op.add_option("--confusion_matrix", action="store_true", dest="print_cm", help="Print the confusion matrix.")
op.add_option("--top10", action="store_true", dest="print_top10", help="Print ten most discriminative terms per class"
                                                                       " for every classifier.")
op.add_option("--all_categories", action="store_true", dest="all_categories", help="Whether to use all categories or "
                                                                                   "not.")
op.add_option("--use_hashing", action="store_true", help="Use a hashing vectorizer.")
op.add_option("--n_features", action="store", type=int, default=2 ** 16, help="n_features when using the hashing "
                                                                              "vectorizer.")
op.add_option("--filtered", action="store_true", help="Remove newsgroup information that is easily overfit: "
...
if opts.select_chi2:
    ...
    if feature_names:
        ...
        for i in ch2.get_support(indices=True):
            feature_names.append(feature_names[i])
...
reg_3 = linear_model.RidgeCV(alphas=np.logspace(-6, 6, 13))
reg_3.fit([[0, 0], [0, 0], [1, 1]], [0, .1, 1])
print(reg_3)
print("reg_3.alpha_: ", reg_3.alpha_)
print("coefficient: ", reg_3.coef_)
